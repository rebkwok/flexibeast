...
def payment_received(sender, **kwargs):
    ...
    try:
        if ipn_obj.payment_status == ST_PP_REFUNDED:
            ...
            user = obj.user.username if obj_type == 'booking' \
                else obj.bookings.first().user
        ...
        if ipn_obj.payment_status == ST_PP_COMPLETED:
            ...
            user = obj.user if obj_type == 'booking' else obj.bookings.first().user
            ActivityLog.objects.create(
                log='{} id {} for user {} has been paid by PayPal; paypal '
                    '{} id {}'.format(
                    obj_type.title(), obj.id, user.username, obj_type,
                    paypal_trans.id
                    )
            )
